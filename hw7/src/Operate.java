...
public class Operate {
    ...
    public void countCommodity(ArrayList<String> input, HashMap<Integer, Adventure> adventures) {
        int advId = Integer.parseInt(input.get(1));
        Adventure adv = adventures.get(advId);
        long price = adv.sumPrice() - adv.getMoney();
        System.out.println(adv.countCommodity() + " " + price);
    }
    ...
    public void sellAllInBag(ArrayList<String> input, HashMap<Integer, Adventure> adventures) {
        int advId = Integer.parseInt(input.get(1));
        Adventure adv = adventures.get(advId);
        adv.sellAllInBag();
    }

    public void buy(ArrayList<String> input, HashMap<Integer, Adventure> adventures) {
        int advId = Integer.parseInt(input.get(1));
        Adventure adv = adventures.get(advId);
        int id = Integer.parseInt(input.get(2));
        String name = input.get(3);
        String type = input.get(4);
        if (input.size() == 5) {
            String other = null;
            adv.buy(id, name, type, other);
        } else {
            String other = input.get(5);
            adv.buy(id, name, type, other);
        }
    }
}
